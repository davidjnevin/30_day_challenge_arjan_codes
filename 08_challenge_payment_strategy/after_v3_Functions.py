...
from after_v1_ABC import PaymentProcessingStrategy
...
PaymentProcessingStrategy = Callable[[Decimal], None]

def PayPalPaymentStrategy(total: Decimal) -> None:
...
def ApplePaymentStrategy(total: Decimal) -> None:
...
def CCPaymentStrategy(total: Decimal) -> None:
    self.total = total
...
def UnKnownPaymentStrategy(total: Decimal) -> None:
